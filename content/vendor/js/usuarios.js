...
var BaseFormValidation = function() {
        ...
        jQuery('#addusuariogenform').validate({
            ...
            rules: {              
                ...
                'usuario' : {
                  ...
                  email: true,
                  remote: { 
                    url: 'UserController/check_user',
                    type: "post",
                    data: { check_user : function(data) { return $("#usuario").val(); } },
                    complete : function(data,status, http){
                          /*var leyend = $(this).validate().settings.messages
                          if (data.responseText ==  '0') {
                            $("#ConRut1-error").text('Rut contacto principal no válido');
                            leyend.ConRut1.remote = "Rut contacto principal no válido";
                          }else if(data.responseText == "false" ){                     
                             $("#ConRut1-error").text('Rut contacto principal ya existe');
                             leyend.ConRut1.remote = "Rut contacto principal ya existe";   
                          }else{
                            $("#ConRut1-error").text('');
                          }
                          $.extend(leyend.ConRut1.remote); */
                      },
  ...
  function addusuario_confirm_files(){
    ...
    //http://stackoverflow.com/questions/5392344/sending-multipart-formdata-with-jquery-ajax
      ...
      var base_url = compensacion+'UserController/adm_Usuarios_Form_Generales_files/'+id_user;
              ...
              //console.log(data.curr_file.Error);
               ...
               if (typeof(data.curr_file) != 'undefined') {//agregado    
                 if (typeof(data.curr_file.curr_url) != 'undefined') {
                  //console.log(data.curr_file); get_down_file base_url
                   //$("#btn_curr").prop( "disabled", false );
                   if ($("#btn_curr").hasClass('disabled')) {
                      $("#btn_curr").removeClass('disabled')
                    };
                   $("#btn_curr").prop( "href", data.curr_file.base_url+"get_down_file/" + id_user +"/curr"  );
                 };  
               }
               if (typeof(data.perfil_file) != 'undefined') {//agregado 
                  if (typeof(data.perfil_file.perfil_url) != 'undefined') {
                    ...
                    $("#btn_perfil").prop( "href", data.perfil_file.base_url+"get_down_file/" + id_user +"/perfil"  );
                  };      
               }
               if (typeof(data.desempeno_file) != 'undefined') {//agregado                                               
                if (typeof(data.desempeno_file.desempeno_url) != 'undefined') {
                  ...
                  $("#btn_desempeno").prop( "href", data.desempeno_file.base_url+"get_down_file/" + id_user +"/desempeno"  );
                }                
               }
               if (typeof(data.foto_file) != 'undefined') {//agregado    
                if (typeof(data.foto_file.foto_url) != 'undefined') {    
                  ...
                  $("#btn_foto").prop( "href", data.foto_file.base_url+"get_down_file/" + id_user +"/foto"  );                                                   
                }
               }
            }else{ //some error
   ...
   function editEscolaridaform(id_esc){ //editar escolaridad // id table #table_esc; id contador #label_find_esc      
      ...
      var desde = $.trim( $(row).find("td:eq(4)").text() );
      var hasta = $.trim( $(row).find("td:eq(5)").text() );
    ...
    function editCursoform(id_cur){      
      ...
      /*$("#cur_ini_dia").val(desde.substring(0, 2));
      $("#cur_ini_mes").val(desde.substring(3, 5));
      $("#cur_ini_ano").val(desde.substring(6, 10));
      
      $("#cur_fin_dia").val(hasta.substring(0, 2));
      $("#cur_fin_mes").val(hasta.substring(3, 5));
      $("#cur_fin_ano").val(hasta.substring(6, 10));*/
