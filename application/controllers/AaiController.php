...
class AaiController extends MY_Controller {
    function __construct(){
        parent::__construct();
        $this->load->model('Aai_model', "aai_m");
    }
    ...
    public function lista_AAI(){
        $data['get_aais'] = $this->aai_m->get_aai();
        $this->load->view('normativa/aai', $data);
    }

    public function form_AAI(){
        $data['getTipo_aai'] = $this->aai_m->data_cat('archivo_tipo');
        $data['get_aais'] = $this->aai_m->get_aai_form();
        $this->load->view('normativa/aai_form', $data);
    }

    public function aai_form_add(){
        $this->form_validation->set_rules('aai_nombre', 'Nombre archivo', 'trim|required|max_length[100]');
        $this->form_validation->set_rules('aai_des', 'Descripción archivo', 'trim|required|max_length[9999]');
        $this->form_validation->set_rules('aai_tipo', 'Tipo archivo', 'trim|max_length[100]');
        if ($this->form_validation->run() == true) {
            $id_aai_form = trim($this->input->post('id_aai_form', TRUE) );
            if (!empty( $id_aai_form )) {
                $data = $this->aai_m->edit_aai();
                $data->id_aai_update = 'update';
            }else{
                $data = $this->aai_m->add_aai();
            }
